...
const vm = new Vue ({
  ...
  async created () {
    this.addNotification('Добро пожаловать!Подождите пока создастся пара ключей.')

    // Initialize crypto webworker thread
    this.cryptWorker = new Worker('crypto-worker.js')//nn

    // Generate keypair and join default room
    this.originPublicKey = await this.getWebWorkerResponse('generate-keys')
    this.addNotification(`Пара ключей сгененрирована - ${this.getKeySnippet(this.originPublicKey)}`)
  ...
  methods: {
    ...
    setupSocketListeners () {
      ...
      this.socket.on('connect', () => {
        this.addNotification('Присоединение к серверу.')
        this.joinRoom()
      })

      // Notify user that they have lost the socket connection
      this.socket.on('disconnect', () => this.addNotification('Соединение разорвано'))

      // дешифрования и прием сообщения
      ...
      this.socket.on('NEW_CONNECTION', () => {
        this.addNotification('Пользователь присоединился к вашей комнате')
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_JOINED', (newRoom) => {
        ...
        this.addNotification(`Текущая комната - ${this.currentRoom}`)
        this.sendPublicKey()
      })

      // Save public key when received
      this.socket.on('PUBLIC_KEY', (key) => {
        this.addNotification(`Открытый ключ разослан - ${this.getKeySnippet(key)}`)
      ...
      this.socket.on('ROOM_FULL', () => {
        this.addNotification(`Нельзя присоединиться, ${this.pendingRoom} комната переполненна`)
      ...
      this.socket.on('INTRUSION_ATTEMPT', () => {
        this.addNotification('Третий пользователь попытался подключиться к комнате')
      })
    },
    ...
    joinRoom () {
      if (this.pendingRoom !== this.currentRoom && this.originPublicKey) {
        this.addNotification(`Присоединение к комнате - ${this.pendingRoom}`)
